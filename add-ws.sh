...
},{"id": "'""$uuid""'","alterId": '"0"',"email": "'""$user""'"' /usr/local/etc/xray/worryfree.json
sed -i '/#grpc$/a\### '"$user $exp"'\
},{"id": "'""$uuid""'","alterId": '"0"',"email": "'""$user""'"' /usr/local/etc/xray/grpc.json
...
echo -e "Path OPOK         : /worryfree"
echo -e "Path Grpc         : vmessgrpc"
echo -e "═══════════════════"
