...
wget -O /usr/local/bin/xray "https://raw.githubusercontent.com/dilathedevilz/ravapath/main/xray.linux.64bit"
chmod +x /usr/local/bin/xray

# generate certificates
mkdir /root/.acme.sh
curl https://raw.githubusercontent.com/dilathedevilz/ravapath/main/acme.sh -o /root/.acme.sh/acme.sh
...
END
# // INSTALLING VMESS Worryfree
cat> /usr/local/etc/xray/worryfree.json << END
{
  "log": {
    "access": "/var/log/xray/access.log",
    "error": "/var/log/xray/error.log",
...
sed -i '$ i{' /etc/nginx/conf.d/xray.conf
sed -i '$ grpc_set_header X-Real-IP \$http_x_forwarded_for;' /etc/nginx/conf.d/xray.conf
sed -i '$ grpc_set_header X-Forwarded-For \$http_x_forwarded_for;' /etc/nginx/conf.d/xray.conf
sed -i '$ client_max_body_size 0;' /etc/nginx/conf.d/xray.conf
sed -i '$ grpc_pass http://127.0.0.1:23457;' /etc/nginx/conf.d/xray.conf
sed -i '$ i}' /etc/nginx/conf.d/xray.conf

sed -i '$ ilocation = /worryfree' /etc/nginx/conf.d/xray.conf
sed -i '$ i{' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_redirect off;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_pass http://127.0.0.1:23455;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_http_version 1.1;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header X-Real-IP \$remote_addr;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header Upgrade \$http_upgrade;' /etc/nginx/conf.d/xray.conf
